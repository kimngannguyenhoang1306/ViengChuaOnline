...
class SearchForm(forms.Form):
    ...
    def search(self):
        ...
        sqs = self.searchqueryset.autocomplete(content_auto1=self.cleaned_data['q'])
        sqs2 = self.searchqueryset.autocomplete(content_auto2=self.cleaned_data['q'])
        sqs3 = self.searchqueryset.autocomplete(content_auto3=self.cleaned_data['q'])
        sqs4 = self.searchqueryset.autocomplete(content_auto4=self.cleaned_data['q'])
        sqs5 = self.searchqueryset.autocomplete(content_auto5=self.cleaned_data['q'])
        sqs6 = self.searchqueryset.autocomplete(content_auto6=self.cleaned_data['q'])
        sqs7 = self.searchqueryset.autocomplete(content_auto7=self.cleaned_data['q'])
        sqs8 = self.searchqueryset.autocomplete(content_auto8=self.cleaned_data['q'])
        sqs9 = self.searchqueryset.autocomplete(content_auto9=self.cleaned_data['q'])
        sqs10 = self.searchqueryset.autocomplete(content_auto10=self.cleaned_data['q'])
        sqs11 = self.searchqueryset.auto_query(self.cleaned_data['q'])
        arr = [sqs2, sqs3, sqs4, sqs5, sqs6, sqs7, sqs8, sqs9, sqs10, sqs11]
        for i in arr:
            if i is None:
                sqs = sqs & i
            else:
